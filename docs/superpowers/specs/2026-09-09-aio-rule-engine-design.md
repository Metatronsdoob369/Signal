...
- **CORS allows credentials (2026-09-11, cosineautonomous.com dogfood).** `navigator.sendBeacon` with an `application/json` Blob runs in `cors` mode with credentials mode `include`, so the browser requires `Access-Control-Allow-Credentials: true` on the beacon response. Without it Chromium logged a CORS error on every real cross-origin client page and discarded the response, even though the server had already stored the audit; the same-origin e2e never exercised the path. `corsHeaders` now sends it. Safe because `Access-Control-Allow-Origin` is always the specific registered-site origin or `APP_ORIGIN`, never `*`, and `/api/pack`, `/api/beacon`, and `/api/resolve` read no cookies. Residual: no cross-origin e2e yet; the dogfood harness intercepts the client origin in Playwright.
